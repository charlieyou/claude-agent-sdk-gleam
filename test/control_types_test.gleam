...
pub fn initialize_request_construction_test() {
  ...
  let Initialize(id, h, servers, checkpointing) = req
...
pub fn incoming_message_control_request_variant_test() {
  ...
  let ControlRequest(r) = msg
  // Verify it's a HookCallback
  case r {
...
pub fn hook_callback_construction_test() {
  ...
  let HookCallback(req_id, cb_id, _, tu_id) = req
...
pub fn can_use_tool_construction_test() {
  ...
  let CanUseTool(req_id, tool, _, suggestions, blocked) = req
...
pub fn mcp_message_construction_test() {
  ...
  let McpMessage(req_id, server, _) = req
...
pub fn hook_response_construction_test() {
  ...
  let HookResponse(id, result) = resp
